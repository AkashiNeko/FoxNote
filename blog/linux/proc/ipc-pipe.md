...
[匿名管道](https://zh.wikipedia.org/wiki/%E5%8C%BF%E5%90%8D%E7%AE%A1%E9%81%93)（Anonymous Pipe）是一种在操作系统中用于进程间通信的机制，它是一种临时的、只能在具有亲缘关系的进程之间使用的无名管道。Linux提供了创建匿名管道的接口 `pipe`。
...
在单进程中，使用匿名管道似乎没有什么意义，它更多情况用于进程间的通信。

在创建匿名管道之后，再使用 `fork()` 创建子进程，这样子进程中就有了和父进程指向相同管道的fd。
...
![父子进程的匿名管道通信](/inset/父子进程匿名管道通信运行结果.svg =560x)
