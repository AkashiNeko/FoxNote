...
在C语言中，结构体用于将不同的类型打包在一起。在C++的类中，不仅有C语言结构体的所有功能，还可以定义函数，以及许多便利的功能，类中的成员函数也称为成员方法（method）。
...
在C++中，使用结构体或类定义变量，不需要像C语言一样使用 `struct` 关键字修饰，可以直接使用类型名。
...
C++使用**访问限定符**关键字限制从类外部对成员的访问，从而达到封装的目的，访问限定符有以下3种。
...
在C语言的结构体中，成员变量会按照内存对齐占用对象空间。那么C++类的成员函数是否会占用对象的空间？
...
从 `sizeof` 的结果来看，类 `A` 和 `B` 占用空间相同。这说明，成员函数并不占用对象的空间。
...
成员函数的调用，可以使用 `object.function()` 的方式。
...
类内的每个非静态成员函数的参数中都隐藏了一个this指针，该指针指向当前调用的对象。
...
static也可以用于修饰成员函数。静态成员函数的参数表中没有this指针，因此静态成员函数不能访问非静态成员变量。
