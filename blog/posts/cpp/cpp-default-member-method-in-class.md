...
类中隐藏了**6个默认的成员函数**（C++11前），如果用户不显示地声明，编译器会自动生成。
...
构造函数是一个隐藏的默认成员函数，其**函数名与类名相同**，且**不写返回值**。
...
因为构造函数是在创建对象的时候调用的，因此它常用于**初始化对象**。

通过**函数重载**，我们可以自己写构造函数来初始化成员变量。
...
如果类的成员变量中存在自定义类型，那么这个类在实例化对象的时候，也会**调用自定义类型的构造函数**。
...
以上的代码中，由于`B`类型中包含`A`的对象，所以在`main`函数中创建`B`类的对象`b1`时，会调用`A`类的构造函数。
...
类初始化时，默认会处理自定义类型，调用自定义类型的构造函数，但是默认不对内置类型进行初始化。在`C++11`中规定：**内置类型在类中声明可以给予初值**。
...
class A {
public:
	explicit A(int a = 0, int b = 2) :_i(a) {}
private:
	int _i;
};
