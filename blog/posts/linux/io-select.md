...
在Linux下，可使用 `man select` 命令查看select的使用方法。
...
timeval是一个表示时长的结构体，有两个成员，一个是秒（tv_sec），另一个是微秒（tv_usec），其表示的时长为二者之和。
...
select接口一共有5个参数，其中除了第1个参数nfds，其他4个都是输入输出型参数。
...
select返回一个整数，表示关注的fd集合中，就绪fd的数量。同时，参数readfds、writefds、exceptfds会被修改为就绪的fd集合。

如果设置了timeout，则timeout会被设置为距离超时时间的剩余时间。比如将超时时间设为5秒，select在等待3秒后返回，则timeout被设置为2秒。

## 3. select服务器

### 业务要求

实现一个简单的select网络服务器，能够接收来自多个客户端的连接，且能够并发地响应客户端的消息。

### 实现思路

使用select同时等待服务器fd和所有的链接fd，如果其中有某个fd就绪了，就处理对应的事件。

下面是select等待多个fd的例子。

![select等待](./p-io-select/select等待.svg)

此时如果有客户端发起连接：

![客户端连接](./p-io-select/客户端连接.svg)

select发现服务器fd有读事件发生，停止等待并返回。此时调用accept接受客户端的连接，并将链接fd放入fd集合中，交给select继续等待。

![获取新连接](./p-io-select/获取新连接.svg)

select可以同时关注多个fd上的IO事件，比如同时接收客户端的连接请求和接收客户端的消息。

![并发响应](./p-io-select/并发响应.svg)
...
1. 用单个执行流实现了并发通信，节省了多执行流调度的开销。
2. 可以同时等待多个文件描述符，相比单个文件描述符的等待，效率更高。
  
### 缺点

1. 实现比较复杂，不便于维护。
2. fd集合需要在用户和内核之间来回拷贝，而且每次都需要遍历所有fd，开销较大。
3. fd集合是一个定长数组，也就算说文件描述符数量有上限。
