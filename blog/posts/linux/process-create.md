...
在之前[进程的状态](/posts/linux/process-status.html#僵尸-z)一文中提到，`fork()` 用于创建一个子进程。它是Linux系统提供的一个创建子进程的接口，它不需要任何参数。
...
调用 `fork()` 时，它在内核中创建一个子进程，与父进程并发执行。当它创建子进程成功时，会为父进程返回子进程的 `PID`，向子进程返回0。如果创建子进程失败，则返回-1。
